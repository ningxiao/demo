(function($){
	...
	var resizeId,container,sections,win = $(window);
	var opts = {},canScroll = true;
	var iIndex = 0,arrElement = [];
	...
	function initPagination(){
		var length = sections.length,pageHtml = '<ul id="pages"><li class="active"></li>';
		if(length){
	...
	function initEffects(dest,element){
		var cssObj,traslate = "",transform = ["-webkit-transform","-ms-transform","-moz-transform","transform"],transition = ["-webkit-transition","-ms-transition","-moz-transition","transition"];
		canScroll = false;
		if(isSuportCss(transform) && isSuportCss(transition)){
			...
			cssObj = (opts.direction == "horizontal")?{left: -dest.left}:{top: -dest.top};
	...
	function reBuild(){
		var offsetTop,offsetLeft,currentHeight = win.height(),currentWidth = win.width();
		var dest,cuerrentElement,element = arrElement[iIndex];
		if(opts.direction == "horizontal"){
		...
		if(iIndex){
			paginationHandler();
			cuerrentElement = arrElement[iIndex];
