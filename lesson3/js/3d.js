window.onload=function ()
{
	...
	for(var i=1;i<=M;i++)
	{
		...
		(function (oNewDiv,i){
			setTimeout(function (){
				utils.setStyle(oNewDiv, {"transform":'rotateY('+(360*(i-1)/M)+'deg) translateZ(400px)'});
				setTimeout(function (){
					if(i==M)fixAll();
					setTimeout(function (){
						utils.setStyle(oNewDiv, {"transition":'none'});						
					},1000);
				}, 3000);
			}, (M+3-i)*200);
		})(oNewDiv,i);
	...
	function fixAll()
	{
		utils.setStyle(oDiv, {"transform":'perspective(1000px) rotateX('+x+'deg) rotateY('+y+'deg)'});		
